...
print('createStackedHistogram version: {}'.format('1.1'))
def createStackedHistogram(data,feature,target,title,caption,course,task,bins=9,values='bool'):
    """
    Create and save a custom stacked histogram
    Input:
    data:
    feature: feature (Numerical)
    target: target binary, yes/no, y/n, t/f, True/False, 1,0
    title:
    caption:
    course:
    task:
    ...
    if values=='tf':
        yes = data[data[target]=='t'][feature]; yes_mean = yes.mean();
        no = data[data[target]=='f'][feature]; no_mean = no.mean()
        ...
        yes = data[data[target]=='y'][feature]; yes_mean = yes.mean();
        no = data[data[target]=='n'][feature]; no_mean = no.mean()  
    ...
    elif values=='AboveBelow':
        yes = data[data[target]=='Above'][feature]; yes_mean = yes.mean();
        no = data[data[target]=='Below'][feature]; no_mean = no.mean()  
        
    else:
        yes = data[data[target]==True][feature]; yes_mean = yes.mean();
        no = data[data[target]==False][feature]; no_mean = no.mean(); 
        
    plt.hist([yes,no], bins=bins, stacked=True, label=data[target])
    title = title + ' ' + str(feature) + ' ' + str(target)
    ax.legend()
